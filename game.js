let data = { talents: [], parents: [], items: [], girls: [], news: [], stocks: [], schools: {}, jobs: [], friends: [], petMarket: [] };
let saves = {};
let player = null;
let currentSave = null;

const STAGE_INFO = [
  { name: "婴儿",    months: 36 },    // 0-3岁
  { name: "幼儿园",  months: 36 },    // 3-6岁
  { name: "小学",    months: 72 },    // 6-12岁
  { name: "初中",    months: 36 },    // 12-15岁
  { name: "高中",    months: 36 },    // 15-18岁
...
function renderParents() {
  ...
  player.parents = p;
  player.familyMoney = p.baseMoney || 20000;
  saveSaves();
...
function renderAttrSelect() {
  ...
  BASIC_ATTRS.forEach(k => {
    ...
    if (attrs[k] < 0) attrs[k] = 0;
    if (attrs[k] > 10) attrs[k] = 10;
  });
...
function nextMonth() {
  ...
  if (player.health < 60 && Math.random() < 0.2) {
    ...
    player.health += 25 + Math.floor(Math.random() * 10);
...
function studySubject(subj) {
  if (player.studyThisMonth) { renderMonthPanel(); return; }
...
function addPet() {
  let types = [
    ...
    { type: "仓鼠", min: 50, max: 120 },
    { type: "乌龟", min: 120, max: 260 },
...
document.addEventListener("DOMContentLoaded", function () {
  let closeBtn = document.getElementById('statusClose');
  if (closeBtn) closeBtn.style.fontSize = "2em";
});
